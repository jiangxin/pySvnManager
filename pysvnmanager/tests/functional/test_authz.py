...
class TestAuthzController(TestController):
    def test_index(self):
        ...
        res = self.app.get(url_for(controller='authz'))
        assert res.status == 302
        self.assertEqual(res.header('location'), 'http://localhost/security/failed', res.header('location'))
...
''' == res.body, res.body
    ...
    def test_delete_authz(self):
        ...
        assert res.status == 302
        assert res.header('location')== 'http://localhost/login', res.header('location')
