...
# how many differ lines in email
my $maxdifflines = 1000;

# max length of subject
my $subject_max_length = 80;
    ...
    if ($arg =~ /^-/)
      {
        ...
        if ($hash_key)
        ...
        else
          {
            if ($arg eq '-m')
              {
                # if defined $current_project->{email_addresses}, then push current_project
                if (@{$current_project->{email_addresses}})
                  {
                    $current_project                = &new_project;
                    $current_project->{match_regex} = $value;
                    push(@project_settings_list, $current_project);
                  }
                # else, set $current_project->{match_regex}
                else
                  {
                    $current_project->{match_regex} = $value;
                  }
...
foreach my $project (@project_settings_list)
  {
    ...
    if (length($subject_base) > $subject_max_length)
      {
        $subject = substr($subject_base, 0, $subject_max_length);
      }
    else
      {
        ...
        if ($from_address =~ /<.*@.*>/)
          {
            ...
            $mail_from =~ s/.*<(.*@.*)>/\1/g;
            $mail_from = "$author <$mail_from>";
          }
        else
          {
            $mail_from = "$author <$from_address>";
          }
      }
    elsif ($hostname =~ /\w/)
      {
        $mail_from = "$author <$mail_from\@$hostname>";
      }
    ...
    if ($diff_wanted and not @difflines)
      {
        ...
        $body_more_notes = "";
        if ($len > $maxdifflines && $maxdifflines > 0)
          {
            $len = $maxdifflines;
